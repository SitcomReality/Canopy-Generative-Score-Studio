// Inline lucide icon SVGs (extracted from lucide-react v1.34.0, ISC license).
// Static markup uses <i data-icon="name" data-size="16"></i> placeholders,
// replaced in place by mountIcons(); dynamically rendered markup calls
// iconSvg() directly.
const PATHS = {
...
// Replace every <i data-icon="..."> placeholder under root with the SVG.
export function mountIcons(root = document) {
  root.querySelectorAll("i[data-icon]").forEach((placeholder) => {
    const size = Number(placeholder.dataset.size) || 16;
    placeholder.outerHTML = iconSvg(placeholder.dataset.icon, size);
  });
}
