...
test("contexts have unique ids, names, and small tempo offsets", () => {
  const ids = new Set();
  for (const context of CONTEXTS) {
    assert.ok(!ids.has(context.id), `duplicate context ${context.id}`);
    ids.add(context.id);
    assert.ok(context.name && context.short && context.icon);
    assert.ok(Number.isInteger(context.tempoOffset) && context.tempoOffset >= 0 && context.tempoOffset <= 40);
  }
